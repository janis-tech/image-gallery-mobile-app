<div class="container mx-auto px-4 py-8">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse ($galleries as $gallery)
            <div class="relative bg-white dark:bg-[#161615] rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-all duration-300 border border-gray-100 dark:border-[#3E3E3A] group">
                <a href="{{ route('galleries.show', $gallery['id']) ?? '#' }}" class="absolute inset-0 z-10">
                    <span class="sr-only">View {{ $gallery['name'] }}</span>
                </a>
                <div class="relative h-48 bg-gray-200 dark:bg-gray-700">
                    @if(isset($gallery['first_image'], $gallery['first_image']['small']))
                    <img 
                        src="{{ $gallery['first_image']['small'] }}" 
                        alt="{{ $gallery['name'] }}" 
                        class="w-full h-full object-cover"
                    >
                    @endif
                    <!-- Image Count Badge -->
                    <div class="absolute bottom-2 right-2 bg-gray-700 bg-opacity-70 dark:bg-black dark:bg-opacity-70 text-white px-2 py-1 rounded-md flex items-center text-sm">
                        <x-flux::icon.layout-grid variant="micro" class="mr-1" />
                        {{ $gallery['images_count'] }} images
                    </div>
                    
                    <div class="absolute top-2 right-2 flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-20">
                        <button 
        ...
        @empty
            <div class="col-span-full py-12 flex flex-col items-center justify-center bg-white dark:bg-[#161615] rounded-lg border border-gray-200 dark:border-[#3E3E3A]">
